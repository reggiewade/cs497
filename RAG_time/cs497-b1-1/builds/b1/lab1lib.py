...
def chat(chat_id, user_message, config=None, rag_query=None):
    ...
    messages = chats[chat_id].copy()

    if rag_query is not None:
        rag_docs = b1lib.rag(rag_query)
        
        context_text = "\n\n".join([doc.page_content for doc in rag_docs])
        
        rag_system_message = SystemMessage(
            content=f"Context information for the query:\n{context_text}"
        )
        
        messages.insert(0, rag_system_message)

    response = llm.chat(messages, config or {})
    if response:
